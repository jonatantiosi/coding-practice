...
class Meta(type):
    def __new__(mcs, name, bases, dct):
        ...
        if 'falar' not in cls.__dict__ \
            or not callable(cls.__dict__['falar']):
            raise NotImplementedError('Implemente falar')

        return cls
    
    def __call__(cls, *args, **kwargs):
        # cria e retorna a instancia
        # o call é responsável por tratar dos parenteses e criar a instancia
        # então aqui tem como ter acesso por exemplo ao dict da propria instancia
